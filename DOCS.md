...
Draw text centered, left-aligned, or right-aligned relative to an anchor point.

**KonScript:**
```ks
# Left-aligned (default — text starts at x)
DrawText("Score: 100", 10.0, 10.0, 24, WHITE);

# Centered on x — great for titles, game over text
DrawTextCentered("GAME OVER", 400.0, 300.0, 48, RED);

# Right-aligned — text ends at x
DrawTextAligned("100 HP", 790.0, 10.0, 20, GREEN, TextAlign.Right);

# Measure text width for manual positioning
let w: F32 = MeasureTextWidth("Hello World", 32);
```

**C++:**
```cpp
DrawTextCentered("GAME OVER", 400, 300, 48, RED);
DrawTextAligned("100 HP", 790, 10, 20, GREEN, TextAlign::Right);
float w = MeasureTextWidth("Hello World", 32);
```

| Function | Description |
|----------|-------------|
| `DrawText(text, x, y, fontSize, color)` | Left-aligned (default) |
| `DrawTextCentered(text, x, y, fontSize, color)` | Centered on x |
| `DrawTextAligned(text, x, y, fontSize, color, align)` | Left / Center / Right |
| `MeasureTextWidth(text, fontSize)` | Returns pixel width of text |

Alignment modes:
```
TextAlign.Left     x is the LEFT edge    |Hello World
TextAlign.Center   x is the CENTER        Hel|lo World
TextAlign.Right    x is the RIGHT edge    Hello World|
```

### Texture Loading

Two filtering modes for different use cases:

```ks
# Pixel art — sharp edges, no blurring (default)
let sprite: Texture = LoadTexture("player.png");

# Smooth — bilinear filtering, for logos/UI/backgrounds
let logo: Texture = LoadTextureSmooth("logo.png");
```

| Function | Filtering | Use for |
|----------|-----------|---------|
| `LoadTexture(path)` | GL_NEAREST (sharp) | Pixel art, sprites, tilesets |
| `LoadTextureSmooth(path)` | GL_LINEAR (smooth) | Logos, UI art, backgrounds |
