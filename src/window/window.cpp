...
static void RunSplashScreen(int w, int h) {
    ...
    Texture logo = {0, 0, 0};
    {
        ...
        if (f) { fclose(f); logo = window->loadTextureSmooth("logo.png"); hasCustomLogo = (logo.id != 0); }
    }
    if (!hasCustomLogo) {
...
Texture Window::loadTextureSmooth(const char* p){return renderer->LoadTextureSmooth(p);}
...
Texture LoadTextureSmooth(const char* p){
    if (!window) return Texture{0,0,0};
    std::string r=AssetManager::resolvePath(p);
    return window->loadTextureSmooth(r.c_str());
}
