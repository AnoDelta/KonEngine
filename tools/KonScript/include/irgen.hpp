...
public:
    ...
    std::string generate(const Program& prog) {
        ...
        for (auto* _inc : m_includes)
            for (auto& s : _inc->stmts) {
                // Skip main() from included files — only the entry file's main is emitted
                if (s->kind == Stmt::Kind::FuncDecl &&
                    static_cast<const FuncDecl*>(s.get())->name == "main")
                    continue;
                genTopLevel(s.get());
            }
        for (auto& s : prog.stmts)
...
private:
    ...
    void emitRuntimeDecls() {
        ...
        emit("declare i8*  @_ks_fmt_float1(double)");
    ...
    void genFunc(const FuncDecl* f) {
        ...
        // Reset alloca hoisting state for this function
        m_allocaBuf.str(""); m_allocaBuf.clear(); m_inFunc = true;
        ...
        // Auto-add return only if not already terminated.
        // An explicit `return` in the body already emitted a terminator;
        // adding another one produces invalid IR and causes crashes.
        if (f->name == "main") {
            if (!blockIsTerminated()) emitI("ret i32 0");
        } else {
            if (!blockIsTerminated()) emitReturnDefault(ret);
        }
    ...
    void genIf(const Stmt* s) {
        ...
        if (!blockIsTerminated()) emitI("br label %" + endL);
    ...
    ValType genExpr(const Expr* e, const TypeAnnotation& hint = {}) {
        ...
        case Expr::Kind::StructInit: {
            ...
            // Look up declared field layout for type-correct stores.
            // Without this, float literals become "double" and the store
            // type-mismatches the float* GEP slot, writing zeros.
            auto sfIt = m_structFields.find(si->typeName);
            for (int i = 0; i < (int)si->fields.size(); i++) {
                // Match by name (init order may differ from declaration)
                std::string fieldLlvmTy;
                int fieldIdx = i;
                if (sfIt != m_structFields.end()) {
                    for (int fi = 0; fi < (int)sfIt->second.size(); fi++) {
                        if (sfIt->second[fi].name == si->fields[i].name) {
                            fieldLlvmTy = sfIt->second[fi].llvmTy;
                            fieldIdx    = fi;
                            break;
                        }
                    }
                    if (fieldLlvmTy.empty() && i < (int)sfIt->second.size())
                        fieldLlvmTy = sfIt->second[i].llvmTy;
                }
                // Build type hint so float literals use the right width
                TypeAnnotation fieldHint;
                if      (fieldLlvmTy == "float")  fieldHint.base = "F32";
                else if (fieldLlvmTy == "double") fieldHint.base = "F64";
                else if (fieldLlvmTy == "i32")    fieldHint.base = "I32";
                else if (fieldLlvmTy == "i64")    fieldHint.base = "I64";
                auto [val, vty] = genExpr(si->fields[i].value.get(), fieldHint);
                // Coerce if there is still a type mismatch (e.g. double→float)
                std::string storeVal = val;
                std::string storeTy  = vty;
                if (!fieldLlvmTy.empty() && vty != fieldLlvmTy) {
                    storeVal = coerce(val, vty, fieldLlvmTy);
                    storeTy  = fieldLlvmTy;
                }
                std::string gep = tmp();
                emitI(gep + " = getelementptr " + sty + ", " + sty + "* " +
                      alloca_ + ", i32 0, i32 " + std::to_string(fieldIdx));
                emitI("store " + storeTy + " " + storeVal + ", " + storeTy + "* " + gep);
            }
            return {alloca_, sty + "*"};
        }
        ...
        case Expr::Kind::Range: {
            // Range expression (a..b or a..=b) — evaluate start and end.
            // In for-in context the ForIn handler already decomposes this;
            // if a Range reaches genExpr it means it's used as a value.
            auto* r = static_cast<const RangeExpr*>(e);
            genExpr(r->from.get());        // evaluate for side effects
            auto [ev, et] = genExpr(r->to.get());
            return {ev, et};               // return end value
        }
        default:
    ...
    ValType genIdent(const IdentExpr* e) {
        ...
        // Bare enum variant: if the name matches a known variant, return its ordinal.
        // This handles `State::Done` via genMember, but also bare `Done` used as a value.
        for (auto& [enumName, variants] : m_enumVariants) {
            auto vit = variants.find(e->name);
            if (vit != variants.end())
    ...
    ValType genCall(const CallExpr* e) {
        ...
        if (callee == "Print") {
            ...
            for (auto& arg : e->args) {
                ...
                // C variadic promotion: float must be extended to double
                // before being passed to printf (varargs don't auto-promote in LLVM IR)
                if (t == "float") {
                    std::string dbl = tmp();
                    emitI(dbl + " = fpext float " + v + " to double");
                    v = dbl; t = "double";
                }
                args.push_back(v);
                ...
                else if (t == "float")  fmt += "%f";   // shouldn't reach here after promotion
                else if (t == "double") fmt += "%f";   // %f for both double and promoted float
                else if (t == "i1")     fmt += "%d";
                else if (t == "i8*")    fmt += "%s";
                else                    fmt += "%p";
            }
        ...
        if (e->callee->kind == Expr::Kind::Member) {
                        ...
                        if (it != m_locals.end()) {
                            ...
                            if (sfit != m_structFields.end()) {
                                for (auto& fi : sfit->second)
                                    if (fi.name == fm->member) return fi.isArray ? "Array" : fi.ksTy;
                            }
                        }
                    }
                }
            ...
            // Skip collection methods if the object is a user-defined struct/class
            // — otherwise method names like "get" collide with _ks_hashmap_get
            bool isUserType = !objKsTy.empty() && m_structFields.count(objKsTy);
            if (cit != collMethods.end() && !isUserType) {
                auto [obj, oty] = genExpr(m->object.get());
    ...
    ValType genMember(const MemberExpr* e) {
        ...
        // Guard: only intercept if the object is NOT a user-defined struct.
        // User structs have llvmTy "%struct.T*"; Result is opaque i8*.
        if (e->member == "ok" || e->member == "value" || e->member == "error") {
            bool isResultType = true;
            if (e->object->kind == Expr::Kind::Ident) {
                auto* id = static_cast<const IdentExpr*>(e->object.get());
                auto lit = m_locals.find(id->name);
                if (lit != m_locals.end() && lit->second.llvmTy.rfind("%struct.", 0) == 0)
                    isResultType = false;
                auto git = m_globalTypes.find(id->name);
                if (git != m_globalTypes.end() && git->second.rfind("%struct.", 0) == 0)
                    isResultType = false;
            }
            if (isResultType) {
