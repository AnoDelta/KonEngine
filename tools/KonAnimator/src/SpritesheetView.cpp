...
void SpritesheetView::setZoom(float z)  { m_zoom = std::max(0.5f, std::min(z, 16.0f)); updateGeometry(); update(); }

QSize SpritesheetView::sizeHint() const {
    if (m_pixmap.isNull()) return QSize(400, 300);
    return QSize((int)(m_pixmap.width() * m_zoom) + 8, (int)(m_pixmap.height() * m_zoom) + 8);
}

QPointF SpritesheetView::widgetToSheet(QPoint p) const {
    return QPointF((p.x() - 4) / m_zoom, (p.y() - 4) / m_zoom);
}

QRect SpritesheetView::frameToWidget(const AnimFrame& f) const {
    int x = (int)(f.srcX * m_zoom) + 4;
    int y = (int)(f.srcY * m_zoom) + 4;
...
void SpritesheetView::paintEvent(QPaintEvent*) {
    ...
    for (int y = 0; y < height(); y += cs)
        for (int x = 0; x < width(); x += cs)
            p.fillRect(x, y, cs, cs, ((x/cs + y/cs) % 2 == 0) ? QColor(55,55,55) : QColor(38,38,38));

    if (m_pixmap.isNull()) {
        p.setPen(QColor(130,130,130));
        p.drawText(rect(), Qt::AlignCenter, "No spritesheet loaded\n\nFile  →  Load Spritesheet");
        return;
    }

    // Sheet
    QRect dst(4, 4, (int)(m_pixmap.width() * m_zoom), (int)(m_pixmap.height() * m_zoom));
    ...
    if (m_dragging) {
        ...
        // Show size in sheet pixels
        QPointF tl = widgetToSheet(drag.topLeft());
        QPointF br = widgetToSheet(drag.bottomRight());
        QString info = QString("%1×%2").arg((int)(br.x()-tl.x())).arg((int)(br.y()-tl.y()));
...
void SpritesheetView::mousePressEvent(QMouseEvent* e) {
    ...
    // Hit test existing frames (reverse order — topmost first)
    if (m_frames) {
...
void SpritesheetView::mouseMoveEvent(QMouseEvent* e) {
    if (m_dragging) { m_dragCurrent = e->pos(); update(); }
}
