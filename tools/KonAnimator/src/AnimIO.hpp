...
// -----------------------------------------------------------------------
// Load .konani binary back into an AnimProject (decompile)
// -----------------------------------------------------------------------
inline bool loadKonani(const std::string& path, AnimProject& proj, std::string& err) {
    std::ifstream f(path, std::ios::binary);
    if (!f.is_open()) { err = "Cannot open: " + path; return false; }

    auto rF = [&]() { float v=0; f.read((char*)&v, 4); return v; };
    auto rU32 = [&]() { uint32_t v=0; f.read((char*)&v, 4); return v; };
    auto rStr = [&]() {
        uint32_t len = rU32();
        std::string s(len, '\0');
        f.read(&s[0], len);
    ...
    auto rU8 = [&]() { uint8_t v=0; f.read((char*)&v, 1); return v; };

    if (!f.good()) { err = "File read error"; return false; }

    proj.clips.clear();
    proj.dirty    = false;

    // Derive a .anim path for the project file path
    auto dot = path.rfind('.');
    proj.filePath = (dot != std::string::npos ? path.substr(0, dot) : path) + ".anim";

    uint32_t clipCount = rU32();
    for (uint32_t i = 0; i < clipCount; i++) {
        ...
        uint32_t frameCount = rU32();
        for (uint32_t j = 0; j < frameCount; j++) {
            AnimFrame fr;
            fr.srcX     = rF(); fr.srcY     = rF();
            fr.srcW     = rF(); fr.srcH     = rF();
            fr.duration = rF();
            c.frames.push_back(fr);
        }

        uint32_t trackCount = rU32();
        for (uint32_t j = 0; j < trackCount; j++) {
            KFTrack tr;
            tr.name = rStr();
            uint32_t keyCount = rU32();
            for (uint32_t k = 0; k < keyCount; k++) {
                Keyframe kf;
                kf.time  = rF();
                kf.value = rF();
                kf.curve = static_cast<Ease>(rU32());
                tr.keys.push_back(kf);
            }
            c.tracks.push_back(tr);
        }

        if (!f.good()) { err = "Unexpected end of file at clip " + std::to_string(i); return false; }
        proj.clips.push_back(c);
    }
    return true;
}
